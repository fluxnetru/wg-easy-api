...
учает версию выпуска WG-Easy. Полезно для проверки, какая версия работает на сервере.
    ...
    /**
     * Gets the WireGuard server configuration backup as text. Retrieves the content of wg0.json.
     * Получает резервную копию конфигурации сервера WireGuard в виде текста.
     * @returns {Promise<Object>} { status: 'success', data: string } (JSON config text) or error | Результат или ошибка
     * @example
     * await api.getBackup()
     * // Returns { status: 'success', data: '{ "interface": { ... } }' }
     */
    async getBackup() {
        return this.call({ method: 'GET', path: '/wireguard/backup' });
    }
}
